            ...
            builder.ApplicationBuilder.Eventing.Subscribe<BeforeStartEvent>(
                async (e, ct) =>
                {
                    ...
                    if (builder.Resource is ContainerResource containerResource)
                    {
                        ...
                        builder
                            .ApplicationBuilder.CreateResourceBuilder(containerResource)
                            .WithBindMount(
                                bindSource,
                                DEV_CERT_BIND_MOUNT_DEST_DIR,
                                isReadOnly: true
                            )
    ...
    )> TryExportDevCertificateAsync(IDistributedApplicationBuilder builder, ILogger logger)
    {
        ...
        try
        {
            try
            {
                ...
                {
                    stdOutTask = ConsumeOutput(
                        exportProcess.StandardOutput,
                        msg => logger.LogInformation("> {StandardOutput}", msg)
                    );
                    stdErrTask = ConsumeOutput(
                        exportProcess.StandardError,
                        msg => logger.LogError("! {ErrorOutput}", msg)
                    );
                }
            }
            catch (Exception ex)
            ...
            else if (!exportProcess.HasExited)
            {
                ...
                logger.LogError(
                    "HTTPS dev certificate export timed out after {TimeoutSeconds} seconds",
